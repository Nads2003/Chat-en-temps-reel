...
export default function Chat() {
  ...
  const renderMessages = () => messages.map((msg, i) => {
    ...
    return (
      <div key={msg.localId || i} className={`flex mb-1 ${isMe ? "justify-end" : "justify-start"}`} >
        {!isMe && (
          <img src={avatarDefault} className="w-7 h-7 rounded-full mr-2 self-end" />
        )}
        <div className={`px-4 py-2 max-w-[75%] text-sm rounded-2xl shadow ${isMe ? "bg-emerald-600 text-white rounded-br-sm" : "bg-[#202c33] text-white rounded-bl-sm"}`} >
          {!isMe && (
        ...
        <div className="fixed inset-0 z-40 md:hidden flex">
          ...
          <div className="relative w-72 bg-[#111b21] h-full z-50">
            <div className="p-4 flex items-center justify-between border-b border-slate-800">
              ...
              <X className="cursor-pointer" onClick={() => setMobileSidebarOpen(false)} />
            </div>
            <div className="flex-1 overflow-y-auto">
              {users.map(user => (
                <div key={user.id} onClick={() => selectUser(user)} className={`flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-[#202c33] ${selectedUser?.id === user.id ? "bg-[#202c33]" : ""}`} >
      ...
      <div className="flex-1 flex flex-col">
        ...
        {selectedUser && (
          <div className="flex items-center gap-2 p-3 bg-[#202c33]">
            ...
            <input value={message} onChange={e => setMessage(e.target.value)} onKeyDown={e => e.key === "Enter" && sendMessage()} placeholder="Message" className="flex-1 px-4 py-2 rounded-full bg-[#2a3942] outline-none" />
            {!recording ? (
              <Mic onClick={startRecording} className="cursor-pointer" />
            ) : (
              <button onClick={stopRecording} className="text-red-500">⏹</button>
            )}
            <button onClick={sendMessage} className="bg-emerald-600 p-2 rounded-full">
              <Send size={18} />
            </button>
          </div>
        )}
